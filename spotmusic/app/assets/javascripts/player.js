$(document).on("turbolinks:load", function(){
  ...
  $("div.play_button a").click(function(){
    button = $(this);
  ...
  $("button#play-all").click(function(){
    button = $("div.song-item:eq(0) div.play_button a");
    playNewSong(button);
  });
  ...
  player.on('ended', function(){
    ...
    next = playing.next();
    if (next.length > 0) {
      play_button = $(next[0]).find('div.play_button a');
  ...
  function playNewSong(button) {
    ...
    $("div.song-item div.play_button i").removeClass("fa-pause-circle");
    $("div.song-item div.play_button i").addClass("fa-play-circle");
  ...
  function pauseSong(button) {
    $(button).children().filter("div.play_button i").removeClass("fa-pause-circle").addClass("fa-play-circle");
    player[0].pause();
  }
